...
//       
//       Added: global variable AnyConstsEnabled;
//              private variables AnyConstsInited, AnyConsts;
//              private function init_any_consts;
//              public functions any_const and any_const_sn;
//              macros (in term.h) MATCH_HINTS_ANYCONST, ANYCONST, 
//                 MAX_ANYCONSTS
//
//       Documentation:
//
//          Any constant matching is globally enabled by macro
//          MATCH_HINTS_ANYCONST (this is supposed to become an input
//          param.  When enabled, hints can contain generic special symbol
//          "_AnyConst" matching any constant (the name can be changed by
//          ANYCONST macro).  Additionally, hints can contain specific
//          symbols "_AnyConst_n" where n is natural number
//          1,...,MAX_ANYCONSTS-1.  Specific symbol "_AnyConst_n" matches
//          also an arbitrary constant but 
//          (a) it must be different from constants matched but other 
//             "_AnyConst_m" (within the same hint), and
//          (b) all occurences of "AnyConst_n" (with the same hint) must
//             match the same constant.
//
//          Note 1: Hints redundancy check
//
//             AnyConst matching is not applied when hints are being
//             tested for redundancy.  "_AnyConst*" symbols behave as
//             ordinary constants (this is achieved using global variable
//             AnyConstsEnabled).  Hence hints "P(_AnyConst)" and "P(a)"
//             are not redundant.  Neither are "P(_AnyConst_1)" and
//             "P(_AnyConst_2)".  Only "P(_AnyConst_1)" and
//             "P(_AnyConst_1)" are redundant.
//          
//          Note 2: Backward hints demodulation
//
//             Backward demodulation of hints with _AnyConst's is
//             problematic.  The following two rules apply to make the
//             behavior predictable.
//
//             Rule A: No demodulation whatsoever is applied to a hint
//                which contains generic "_AnyConst".  This is to prevent
//                hint "P(f(_AnyConst,_AnyConst))" to be simplified with
//                demulator "f(x,x)=a" to "P(f(a))".
//
//             Rule B: However, backward demodulation is applied to hints
//                with specific "_AnyConst_n", so hint
//                "P(f(_AnyConst_1,_AnyConst_1))" can be simplified by
//                "f(x,x)=a" to "P(f(a))".  But the hint
//                "P(f(_AnyConst_1,_AnyConst_2))" can not be simplified by
//                the above demodulator.
//
//       Implementation notes:
//
//       * AnyConstsEnabled can be used to temporarily enable/disable 
//         AnyConsts matching from the code
//       * MATCH_HINTS_ANYCONST is global macro to enable/disable at 
//         all.  This is supposed to be tuned into user parameter.  When
//         this is FALSE, AnyConstsEnabled changes have no effect.
//       * Any constant matching is so far implemented only for FPA indexing
//         structure (see fpa.c).  It will not work if hints indexing is 
//         changed to discrimination trees, for example.
//       * Term matching and subsumption is extended, so that they remember
//         bindings of _AnyConst_n to specific constants.  See match_anyctx 
//         (unify.c) and subsume_literals_anyctx (subsume.c).
//       * AnyConst symbol identifier ("_AnyConst") can be changed by ANYCONST
//         macro.  If you change it so that possible symbol length can be
//         more than 64 characters, then change the "str" array size in
//         init_any_consts.
//
//
// ***************************************************************************
...
/* JJ(2016-nov-10): Match any constant (_AnyConst) in hints */
BOOL AnyConstsEnabled = TRUE; /* temporarily enable/disable _AnyConst matching */
...
/* JJ(2016-nov-10): Match any constant (_AnyConst) in hints */
static int AnyConsts[MAX_ANYCONSTS]; /* symbol numbers of "_AnyConsts_n" */
 ...
 *   JJ(2016-nov-10): Match any constant (_AnyConst) in hints
 *      
 *      Initialize AnyConst array with _AnyConst symbol numbers.
 *
 *************/
...
/* DOCUMENTATION
Return n when sn is the symbol number AnyConst_n.  When sn is the  
symbol number of generic _AnyConst, return 0.  Return -1 otherwise.
*/
...
/* DOCUMENTATION
Return the symbol number of special constant _AnyConst_n.  
For n==0, return the symbol number of generic _AnyConst.
*/
